...
public class Copier extends JPanel implements ActionListener, PropertyChangeListener {
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                JFrame frame = new JFrame("GUI Copier");
                // application will exit on close
                frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
                frame.setSize(DEFAULT_WIDTH, DEFAULT_HEIGHT);
                
                // create example app content pane
                // Copier constructor does additional GUI setup
                JComponent contentPane = new Copier();
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        File srcDir = new File("in");  // input directory path
        if (srcDir.exists() && (srcDir.listFiles() != null && srcDir.listFiles().length > 0)) {
            // set up the destination directory
            File destDir = new File("out");   // output directory path           
            // create the progress monitor
            progressMonitor = new ProgressMonitor(Copier.this,
            ...
            // add Copier as a listener on CopyFiles;
